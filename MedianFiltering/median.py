...
save_path = '/media/samir/Secondary/Denoising/median results/'
kernel_size = 5

dirs = os.listdir(data_path)
num_images = len(dirs)
for i in range(num_images):
	print("Median Filtering Image:", i+1)
	...
	denoised_r = median_filter(noisy[:,:,0], kernel_size)
	denoised_g = median_filter(noisy[:,:,1], kernel_size)
	denoised_b = median_filter(noisy[:,:,2], kernel_size)
...
# Execution Time: 4612.018218755722 s